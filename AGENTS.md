...
## Назначение файлов AGENTS.md и GEMINI.md
Данный файл (`AGENTS.md`) является единым источником истины (Source of Truth) для всех используемых в проекте ИИ-ассистентов (GitHub Copilot, Cursor, Windsurf, Claude и др.).
Файл `GEMINI.md` представляет собой символическую ссылку (symlink) на `AGENTS.md`, что гарантирует полную синхронизацию системных инструкций для моделей Google Gemini.

**Что должно храниться в этом файле (Best Practices):**
1. **Роль и Контекст:** Описание проекта, стек технологий, целевая ОС и архитектура.
2. **Дизайн-система:** Строгие правила построения UI (компоненты, отступы, материалы), чтобы ИИ не создавал UI с нуля, а переиспользовал существующий (напр., "Tahoe Look").
3. **Стандарты Кода:** Правила оформления кода, нейминг, обработка ошибок и особенности конкретной версии языка (Swift 6 Strict Concurrency).
4. **Архитектурные Паттерны:** Указания на используемые подходы (MVVM, инъекция зависимостей).
5. **Границы дозволенного (Boundaries):** Строгие запреты (например, "не использовать force unwrap", "правила работы с фокусом окна").
6. **Workflows:** Пошаговые алгоритмы для частых задач (например, создание новых страниц, дебаг записи аудио).

*Любые изменения в архитектуре, правилах или стандартах UI должны в первую очередь вноситься сюда!*
